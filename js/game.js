window.onload = () => {
    const game = {
        init: function () {
            ...
            this.timeStart = 0;
            document.getElementById('elapsed-time-counter').innerText = 0
        ...
        drawBoard: function () {
            ...
            this.rows = parseInt(urlParams.get('size'));
            this.cols = this.rows
        ...
        updateFlags: function(incValue) {
            ...
            if (newCount >= 0 && newCount <= this.mineCount) {
                ...
                elem.innerText = this.availableFlagsCount
                return true
            }
            return false
        },
    ...
    startButton.addEventListener('click', (e) => {
        ...
        const mines = prevSibling.querySelector('input').value
        prevSibling = prevSibling.previousElementSibling
        const size = prevSibling.querySelector('input').value

        window.location.hash = `?size=${size}&mines=${mines}`
        game.init();
    })
}
